...
import { CreateVocabularyDto } from '@vocabulary/types';
import { api } from '@/lib/api';
import { Sparkles, Loader as Loader2, CircleAlert as AlertCircle } from '@keyline-icons/react';

interface AddWordDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  activePair: LanguagePair;
  onSubmit: (dto: CreateVocabularyDto) => Promise<void>;
}
...
export function AddWordDialog({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await api.vocabulary.check(trimmed, activePair.source);
        if (res.exists && res.vocabulary) {
          setDuplicateWarning(
            `"${trimmed}" is already in your ${activePair.label} vocabulary (Status: ${res.vocabulary.status}, Mastery: ${Math.round(res.vocabulary.masteryScore)}%).`,
          );
        } else {
          setDuplicateWarning(null);
        }
  ...
  }, [word, activePair.source, activePair.label]);

  const handleWordChange = (val: string) => {
    if (activePair.source === 'de') {
      const match = val.match(/^(der|die|das)\s+(.+)$/i);
      if (match) {
    ...
    try {
      ...
      if (entry) {
        ...
        if (entry.translations && Array.isArray(entry.translations) && entry.translations.length > 0) {
          const match = entry.translations.find((t: any) => t.targetLanguage === activePair.target);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedWord) {
      ...
      setError('Please enter a word to save.');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Failed to save word. Please check your input.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <DialogContent className="sm:max-w-[520px] p-0 gap-0 overflow-hidden rounded-2xl border border-border bg-card shadow-xl">
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          <div className="p-5 pr-12 border-b border-border/70 bg-muted/20">
            <DialogHeader className="gap-1">
              ...
              <DialogDescription className="text-xs text-muted-foreground">
                Saving to{' '}
                <strong className="text-foreground">{activePair.label}</strong>.
                New words start in <span className="font-medium text-foreground">Saved</span>{' '}
                status.
              </DialogDescription>
            </DialogHeader>
          </div>
          ...
          <div className="p-5 space-y-3.5 max-h-[62vh] overflow-y-auto">
            ...
            {duplicateWarning && (
              <div className="rounded-md banner-warning p-2.5 text-xs flex items-center gap-2">
            ...
            {/* German Article Selector (der / die / das) */}
            ...
            <div className="grid grid-cols-3 gap-2.5">
              <div className="col-span-2 space-y-1">
                ...
                <Input
                  ...
                  placeholder={
                    activePair.source === 'de' ? 'e.g. Hund or Buch' : 'e.g. resilient'
                  }
                  required
                  autoFocus
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                placeholder={
                  activePair.target === 'en'
                    ? 'e.g. dog / to decide'
                    : 'e.g. কুকুর / সিদ্ধান্ত নেওয়া'
                }
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                placeholder="https://spiegel.de/artikel/..."
                type="url"
